...
- [ ] **Performance profiling** under **larger fixture counts / high modulator density** — **partial:** profiler snapshot + Settings show fixture/modulator/logical-universe counts, **max build/send/total**, **nine-bucket total-time histogram**, **approx. median / p95** for **total**, **build**, and **send** (interpolated from bins), and **Reset stats**. **Audit note (2026-07-16):** exact ring-buffer quantiles (≤512 samples) for total/build/send are **already implemented** in `DMXPerformanceProfiler` and exposed on `/api/state`; Settings surfaces the exact **total** median/p95 only. Remaining: surface exact build/send in Settings UI (or accept API-only) and optional large-rig soak evidence.

**Audit corrections (2026-07-16) — honest transport boundaries to reflect in UI/docs:**

- **Universe offset asymmetry:** outbound adds `dmxNetworkUniverse` as an offset to each logical universe, but **inbound merges by raw wire universe** (no offset translation). Operators using an output offset must align their listen range / fixture logical universes manually.
- **Multi-universe software scope:** cues, modulation, legacy visualization, and haze emergency-kill currently apply to **logical universe 0 only** (`DMXUniverseBuilder`). Non-zero universes receive fixture manual values only.
- **Outbound sACN destination:** all universes are sent to a **single configured host** (`dmxSACNHost`); the app does **not** send each universe to its own `239.255.hi.lo` multicast group (inbound *does* join per-universe multicast).
- **RDM discovery:** the **hardware** path is a deterministic **mock**, but **Art-Net/sACN** modes perform a **real ArtPoll node discovery** (UDP; nodes/short-name/MAC→UID) — not RDM GET/SET. "Mock only" is accurate for the USB/hardware path, not the network path.
