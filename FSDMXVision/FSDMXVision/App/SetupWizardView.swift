    ...
    private var stepContent: some View {
        ...
        case "welcome":
            VStack(alignment: .leading, spacing: 8) {
                Text("Welcome to \(AppIdentity.displayName) \(AppBuildInfo.betaLabel).")
        ...
        case "audio":
            VStack(alignment: .leading, spacing: 8) {
                ...
                Text("\(AppIdentity.displayName) needs microphone access for audio-reactive visuals. If macOS blocks access after a new build/install, use the button below to reopen Privacy settings.")
                    .font(.caption2)
                    .foregroundStyle(.secondary)
                HStack(spacing: 8) {
