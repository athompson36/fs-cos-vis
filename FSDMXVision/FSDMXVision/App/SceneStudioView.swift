...
struct SceneStudioView: View {
    ...
    private var fractalUniverseControlsCompact: some View {
        VStack(alignment: .leading, spacing: 8) {
            Picker("Geometry", selection: fractalGeometryBinding) {
                ...
                Text("Newton (z³ − 1)").tag(5)
                Text("Orbit trap").tag(6)
            }
            .controlSize(.small)
            ...
            studioCaptionSlider(title: "Spectrum warp", value: spectrumWarpAmountBinding, in: 0 ... 1)
    ...
    private var fractalGeometryBinding: Binding<Int> {
        Binding(
            ...
            set: { v in
                appModel.applyCurrentLayerEdit { $0.fractalGeometryIndex = Float(max(0, min(6, v))) }
            }
        )
    }

    private var spectrumWarpAmountBinding: Binding<Float> {
        Binding(
            get: {
                guard let id = appModel.selectedSceneID else { return 0.35 }
                return appModel.sceneEditStates[id]?.layer.spectrumWarpAmount ?? 0.35
            },
            set: { v in
                appModel.applyCurrentLayerEdit { $0.spectrumWarpAmount = max(0, min(1, v)) }
            }
        )
    }
