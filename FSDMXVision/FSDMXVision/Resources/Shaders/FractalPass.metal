...
static float2 cdiv(float2 a, float2 b) {
    float den = dot(b, b);
    if (den < 1e-14f) {
        return float2(0.0f);
    }
    return cmult(a, float2(b.x, -b.y)) / den;
}
...
fragment float4 fractalFragment(VertexOut in [[stage_in]],
    ...
    if (geo == 0) {
        ...
        // Newton fractal for z³ − 1 (basin coloring via final z).
        z = p;
        for (; i < maxIter; i++) {
            float2 z2 = csquare(z);
            ...
            float den = dot(fp, fp);
            if (den < 1e-12f) { break; }
            float2 step = cdiv(f, fp);
            z = z - step;
            if (dot(step, step) < 1e-10f) { break; }
        }
    } else if (geo == 6) {
        // Mandelbrot + vertical line orbit trap (|Re(z) − trap|).
        orbitTrapMin = 1e9f;
        c = p + float2(0.05f * sin(u.time * 0.12f), 0.0f);
        z = float2(0.0f, 0.0f);
        float trapLineX = 0.22f * sin(u.time * 0.55f + pulse * 2.0f);
        for (; i < maxIter; i++) {
            orbitTrapMin = min(orbitTrapMin, fabs(z.x - trapLineX));
            if (dot(z, z) > 4.0f) { break; }
            z = csquare(z) + c;
        }
    }
    ...
    float tSmooth;
    if (geo == 5) {
        float2 r0 = float2(1.0f, 0.0f);
        float2 r1 = float2(-0.5f, 0.8660254f);
        float2 r2 = float2(-0.5f, -0.8660254f);
        float d0 = length(z - r0);
        float d1 = length(z - r1);
        float d2 = length(z - r2);
        float basinSel = (d0 <= d1 && d0 <= d2) ? 0.0f : ((d1 <= d2) ? 1.0f : 2.0f);
        float iterN = float(i) / float(max(maxIter, 1));
        float nearest = min(d0, min(d1, d2));
        float conv = 1.0f - saturate(nearest * 5.5f);
        tSmooth = saturate(iterN * 0.55f + conv * 0.35f + basinSel * 0.1f);
    } else if (geo == 6) {
        float tm = max(orbitTrapMin, 1e-8f);
        float escT = float(i) / float(max(maxIter, 1));
        float trapT = saturate(1.0f - log(1.0f + tm * 140.0f) / 5.8f);
        tSmooth = mix(escT, trapT, 0.62f);
        tSmooth = saturate(tSmooth);
    } else if (i >= maxIter) {
        tSmooth = 1.0f;
    } else {
