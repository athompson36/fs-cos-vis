...
private enum LightingWorkspaceSection: String, CaseIterable, Identifiable {
    case patch
    case cues
    case stage
    case verify
    ...
    var systemImage: String {
        switch self {
        case .patch: "cable.connector"
        case .cues: "list.bullet.rectangle"
...
struct LightingWorkspaceView: View {
    ...
    @AppStorage("lighting.workspace.section") private var lightingSectionRaw = LightingWorkspaceSection.patch.rawValue

    private var lightingSection: LightingWorkspaceSection {
        LightingWorkspaceSection(rawValue: lightingSectionRaw) ?? .patch
    }

    var body: some View {
        VStack(alignment: .leading, spacing: 0) {
            lightingWorkspaceSectionBar
            Group {
                switch lightingSection {
                case .patch:
                    patchWorkspaceScroll
                case .cues:
                    cuesWorkspaceScroll
                case .stage:
                    stageWorkspaceScroll
                case .verify:
                    verificationWorkspaceTab
                case .tools:
                    toolsWorkspaceScroll
                }
            }
    ...
    private var lightingWorkspaceSectionBar: some View {
        VStack(alignment: .leading, spacing: 6) {
            Text("Lighting workspace")
                .font(.caption.weight(.semibold))
                .foregroundStyle(.secondary)
            ScrollView(.horizontal, showsIndicators: false) {
                HStack(spacing: 6) {
                    ForEach(LightingWorkspaceSection.allCases) { section in
                        lightingWorkspaceChip(section)
                    }
                }
                .padding(.vertical, 2)
            }
    ...
    private func lightingWorkspaceChip(_ section: LightingWorkspaceSection) -> some View {
        let selected = lightingSection == section
        return Button {
            lightingSectionRaw = section.rawValue
        } label: {
    ...
    private var patchWorkspaceScroll: some View {
        ScrollView {
    ...
    private var cuesWorkspaceScroll: some View {
        ScrollView {
            VStack(alignment: .leading, spacing: 20) {
                cueSection
                backdropCueSection
    ...
    private var stageWorkspaceScroll: some View {
        ScrollView {
            VStack(alignment: .leading, spacing: 20) {
                StagePlanView()
    ...
    private var toolsWorkspaceScroll: some View {
        ScrollView {
            VStack(alignment: .leading, spacing: 20) {
                modulationSection
