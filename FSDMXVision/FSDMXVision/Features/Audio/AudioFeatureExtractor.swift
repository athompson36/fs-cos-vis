...
enum AudioFeatureExtractor {
    ...
    /// Maps FFT magnitude bins to `binCount` buckets with exponentially spaced frequency edges (more resolution in mids/highs).
    static func binnedMagnitudes(magnitudes: [Float], binCount: Int = 16) -> [Float] {
        guard magnitudes.count >= 2, binCount > 0 else { return Array(repeating: 0, count: max(1, binCount)) }
        let n = magnitudes.count
        var out = [Float](repeating: 0, count: binCount)
        let maxIdx = Float(n - 1)
        for b in 0..<binCount {
            let t0 = powf(maxIdx, Float(b) / Float(binCount))
            let t1 = powf(maxIdx, Float(b + 1) / Float(binCount))
            var i0 = Int(t0)
            var i1 = Int(ceil(t1))
            i0 = max(0, min(n - 1, i0))
