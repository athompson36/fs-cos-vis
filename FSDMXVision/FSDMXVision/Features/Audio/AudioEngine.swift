...
final class AudioEngine: ObservableObject, @unchecked Sendable {
    ...
    func stop() {
        ...
        lastBPM = 0
        lastConfidence = 0
        spectrumSmooth = Array(repeating: 0, count: 16)
        lastSpectrum16 = Array(repeating: 0, count: 16)
    }
    ...
    private func handle(buffer: AVAudioPCMBuffer) {
        ...
        while fftAccumulator.count >= fftSize {
            ...
            let rawBins = AudioFeatureExtractor.binnedMagnitudes(magnitudes: mags, binCount: 16)
            for i in 0..<16 {
                spectrumSmooth[i] = spectrumSmooth[i] * 0.85 + rawBins[i] * 0.15
            }
            let peakS = spectrumSmooth.max() ?? 1e-8
            if peakS > 1e-12 {
                lastSpectrum16 = spectrumSmooth.map { $0 / peakS }
            }
        }
        ...
        next.spectrum16 = lastSpectrum16
