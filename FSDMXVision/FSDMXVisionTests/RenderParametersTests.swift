...
final class RenderParametersTests: XCTestCase {
    ...
    func testUniforms_spectrumWarpAndDyeViscosity_passedThrough() {
        var p = RenderParameters()
        p.spectrum16 = (0 ..< 16).map { Float($0) / 15.0 }
        p.spectrumWarpAmount = 0.42
        p.dyeLayerViscosity[0] = 0.11
        p.dyeLayerViscosity[7] = 0.88
        let u = p.uniforms(drawableSize: CGSize(width: 32, height: 32))
        XCTAssertEqual(u.spectrum0.x, 0, accuracy: 0.0001)
        XCTAssertEqual(u.spectrum3.w, 1, accuracy: 0.0001)
        XCTAssertEqual(u.spectrumWarp.x, 0.42, accuracy: 0.0001)
        XCTAssertEqual(u.dyeViscosity0.x, 0.11, accuracy: 0.0001)
        XCTAssertEqual(u.dyeViscosity1.w, 0.88, accuracy: 0.0001)
        XCTAssertEqual(MemoryLayout<CosmicUniforms>.stride % 16, 0)
    }
}
